...
class CPgsqlSchema extends CDbSchema
{
	...
	{
		$sql=<<<EOD
SELECT a.attname, LOWER(format_type(a.atttypid, a.atttypmod)) AS type, pg_get_expr(d.adbin, adrelid), a.attnotnull, a.atthasdef,
...
EOD;
		...
		foreach($columns as $column)
		{
			...
			if(stripos($column['pg_get_expr(d.adbin, adrelid)'],'nextval')===0 && preg_match('/nextval\([^\']*\'([^\']+)\'[^\)]*\)/i',$column['pg_get_expr(d.adbin, adrelid)'],$matches))
			{
	...
	protected function createColumn($column)
	{
		...
		$c->init($column['type'],$column['atthasdef'] ? $column['pg_get_expr(d.adbin, adrelid)'] : null);
